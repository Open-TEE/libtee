 ...
 * | sequence	| length	| 64/32bit	| arbitary	|
...
struct com_msg_open_session {
	...
	TEE_Result return_code_create_entry; /* 4 bytes */
	TEE_Result return_code_open_session; /* 4 bytes */
	uint32_t return_origin;
	uint32_t sess_fd_to_caller; /* 4 bytes */
...
struct com_msg_invoke_cmd {
	...
	uint32_t return_origin;
#ifdef ENV64BIT
	char padding[4]; /* 8 byte algiment */
#endif
	/* parameters */
};
 ...
 * verifys message. Function have a timeout mechanism. If function is interrupted
 * by EINTR, function recall read function.
 * \param sockfd
 ...
 * Same as com_wait_and_recv_msg, but function do not have timeout mechanism
 * \param sockfd
 * \param msg
 * \param msg_len
 * \return
