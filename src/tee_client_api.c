					      ...
					      enum mem_type type)
{
	...
	internal_imp = (struct shared_mem_internal *)malloc(sizeof(struct shared_mem_internal));
	if (!internal_imp) {
					  ...
					  struct com_msg_operation *internal_op)
{
	...
	for (i = 0; i < 4; i++) {
		if (TEEC_PARAM_TYPE_GET(param_types, i) == TEEC_NONE) {
			...
			if ((parent = operation->params[i].memref.parent)) {
				/* We have some shared memory area */
				internal_imp = (struct shared_mem_internal *)parent->imp;
				if (internal_imp->type == REGISTERED) {
					 ...
					 */
					if (!parent->buffer || !internal_imp->reg_address)
						continue;

					memcpy(parent->buffer, internal_imp->reg_address,
					       parent->size);
				}
				...
				internal_op->params[i].memref.size = parent->size;
			}
		}
	}
}
...
TEEC_Result TEEC_InitializeContext(const char *name, TEEC_Context *context)
{
	...
	inter_imp = (struct context_internal *)malloc(sizeof(struct context_internal));
	if (!inter_imp) {
			     ...
			     uint32_t *return_origin)
{
	...
	session_internal = (struct session_internal *)malloc(sizeof(struct session_internal));
	if (!session_internal) {
